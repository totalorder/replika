...
class Scanner(threading.Thread):
    def __init__(self, db_name, output_queue, db_factory=SqliteDict):
        super().__init__()
        self.setDaemon(True)
        self.output_queue = output_queue
        self.paths_to_scan = []
        self.pool = Pool(3)
        self.db = db_factory('{}.sqlite'.format(db_name), autocommit=True)
    ...
    def add_path(self, sync_point, path):
        sentinel = object()
        apply_result = self.pool.apply_async(self.scan_path,
                                             (sentinel, sync_point, path))
        self.scans_in_progress[sentinel] = apply_result

    def scan_path(self, sentinel, sync_point, root):
        for path, subdirs, files in os.walk(root):
            for name in files:
                self.paths_to_scan.append((sync_point,
                                           False,
    ...
    def step(self):
        ...
        if self.paths_to_scan:
            sync_point, is_dir, path = self.paths_to_scan.pop(
                random.randint(0, len(self.paths_to_scan) - 1))
            mtime = getmtime(path)
            ...
            if record_is_new or record['size'] != size:
                self.output_queue.put((record['sync_points'], path, is_dir,
                                       mtime, size, False))
            self.db[path] = record
            ...
            path = self.deletes_to_check.pop(
                random.randint(0, len(self.deletes_to_check) - 1))
            record = self.db.get(path, None)
