...
class Network(object):
    def __init__(self, client_accepted_cb):
        self.clients = {}
        self.logger = HierarchyLogger(lambda: "Network")
        self.listeners = {}
        self.client_accepted_cb = client_accepted_cb

    @async.task
    def connect(self, address):
        reader, writer = yield from asyncio.open_connection(*address)

        return self._create_client(reader, writer)


    def _create_client(self, reader, writer):
        client = Client(writer.transport.get_extra_info('peername'), reader, writer, False)
        self.clients[((reader, writer))] = client
        return client

    def _connection_accepted_cb(self, reader, writer):
        self.client_accepted_cb(self._create_client(reader, writer))
