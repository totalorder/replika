...
class SyncPoint(object):
    ...
    def signal(self, source_path, event_type):
        self.logger.info(u"Received signal: %s - %s", source_path, event_type)
    ...
    def _send_event(self, evt):
        if evt.type in self.outstanding_events[evt.source_path]:
            self.logger.info(u"Filtered out evt %s because %s", evt, self.outstanding_events[evt.source_path])
    ...
    def start(self, observer):
        if self.started:
            ...
            self.logger.info(u"Observing directory: %s", self.mount_path)
            return True
